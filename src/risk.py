...
FLOOR = 0.5   # only queue transactions the model flags as probable fraud
...
def alert_budget(scored, ks=(100, 500, 1000, 5000)):
    """Top-K of the gated queue (p_fraud >= FLOOR), ranked by expected loss."""
    queue = (scored[scored["p_fraud"] >= FLOOR]
             .sort_values("expected_loss", ascending=False)
             .reset_index(drop=True))
    total_fraud = int(scored["is_fraud"].sum())                  # vs ALL fraud
    ...
    for k in ks:
        ...
        rows.append({
            "alerts_reviewed":    min(k, len(queue)),
            "fraud_caught":       caught,
    ...
    return pd.DataFrame(rows), queue
...
def main():
    ...
    table, queue = alert_budget(scored)
    print(f"Alert queue: {len(queue):,} txns above p_fraud >= {FLOOR}, "
          f"ranked by expected loss\n")
    print(table.to_string(index=False))

    out_cols = ["trans_num", "trans_time", "cc_num", "category",
                "amt", "p_fraud", "expected_loss", "is_fraud"]
    queue[out_cols].to_parquet("data/processed/alerts.parquet", index=False)
    print(f"\nSaved data/processed/alerts.parquet ({len(queue):,} alerts)")
